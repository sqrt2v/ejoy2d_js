    ...
    return {
       /**
        ...
        request: function (method, url, options, xhr) {
            ...
            if (options.postdata != null) {
                if (options.postdata instanceof Document) {
                    ...
                    if(!ejoy2d.isDefined(contentType)) {
            ...
            if (options.query) {
                uri = new ejoy2d.URI(url);
                query = ejoy2d.extend(uri.getQuery(), options.query);
                uri.setQuery(query);
                url = uri.toString();
            }
        ...
        guessResponseType: function (url) {
            var uri = new ejoy2d.URI(url);
            var ext = ejoy2d.path.getExtension(uri.path);
        ...
        isBinaryContentType: function (contentType) {
            var binTypes = [ContentType.WAV, ContentType.OGG, ContentType.MP3, ContentType.BIN, ContentType.DDS];
        ...
        onSuccess: function (method, url, options, xhr) {
            ...
            if (contentType === ContentType.JSON || ejoy2d.string.endsWith(url, ".json")) {
                // It's a JSON response
                response = JSON.parse(xhr.responseText);
            } else if (this.isBinaryContentType(contentType)) {
                response = xhr.response;
            } else {
                if (xhr.responseType === ResponseType.ARRAY_BUFFER) {
                    logWARNING(ejoy2d.string.format('responseType: {0} being served with Content-Type: {1}', ResponseType.ARRAY_BUFFER, contentType));
                    response = xhr.response;
                } else {
                    if (xhr.responseType === ResponseType.DOCUMENT || contentType === ContentType.XML) {
